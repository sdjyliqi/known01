...
func Test_CardID(t *testing.T) {
	...
	t.Log(time.Now())
	text := "我们在使用分列功能进行出生日期提取之前，先来看看身份证号码的组成：二代身份证都是18位，前面6位152102198510281834中文是地区代码，中间8位是出生年月日，后面三位是编码，最后一位是识别码。身份证号012152102198510281834中文"
	for i := 1; i < 1; i++ {
		ExtractCardIDs(text)
	}
...
func Test_PhoneID(t *testing.T) {
	text := "我们在使14012346789用分列功能进1521051028578行出生日期提取之前，我手机号15210510285,0.115210510285，   18701516837开心"
